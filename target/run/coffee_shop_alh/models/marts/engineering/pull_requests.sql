  ...
  create or replace view `aec-students`.`dbt_alh`.`pull_requests`
  OPTIONS()
  as with 

pull_request as (
    select * from `aec-students`.`dbt_alh`.`stg_github_pull_request`
), 
repositories as (
    select * from `aec-students`.`dbt_alh`.`stg_github_repository`
),issues as (
    select * from `aec-students`.`dbt_alh`.`stg_github_issue`
),issues_merged as (
    select * from `aec-students`.`dbt_alh`.`stg_github_issue_merged`
),final as (
